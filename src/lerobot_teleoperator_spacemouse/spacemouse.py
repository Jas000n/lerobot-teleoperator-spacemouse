...
ACTION_KEYS = ("enabled", *AXIS_ACTION_KEYS, "gripper_vel")
...
def state_to_action(state: Any | None, cfg: SpaceMouseTeleopConfig, *, now: float | None = None) -> RobotAction:
    ...
    # The device only reports on change, so holding a button sends nothing further and the state
    # goes stale within `input_timeout_s`. The timeout exists to stop axis drift after the puck is
    # released, so it must zero the axes only: clearing the buttons as well would freeze the
    # gripper (and `enable_button`) unless the puck happens to be moved at the same time.
    if state_is_stale(state, cfg, now=now):
        axes = dict.fromkeys(AXIS_ACTION_KEYS, 0.0)
    else:
        axes = {
    ...
    action = {**axes, "gripper_vel": gripper_vel}
