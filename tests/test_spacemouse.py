...
def test_lerobot_factory_instantiates_concrete_spacemouse_teleoperator(tmp_path):
    config = SpaceMouseTeleopConfig(id="test", calibration_dir=tmp_path)

    teleop = make_teleoperator_from_config(config)

    assert isinstance(teleop, SpaceMouseTeleop)
    assert not isabstract(SpaceMouseTeleop)
    assert tuple(teleop.action_features) == ACTION_KEYS
...
def test_stale_state_keeps_buttons_live():
    # A held button produces no new HID reports, so the state goes stale while the gripper command
    # must stay active. Only the axes are zeroed.
    cfg = SpaceMouseTeleopConfig(deadzone=0.0, input_timeout_s=0.08, gripper_open_button=1)
    state = SimpleNamespace(t=1.0, x=1.0, y=0.0, z=0.0, roll=0.0, pitch=0.0, yaw=0.0, button=[0, 1])

    action = state_to_action(state, cfg, now=1.2)

    assert action["gripper_vel"] == 1.0
    assert action["enabled"] is True
    assert all(action[key] == 0.0 for key in ("target_x", "target_y", "target_z"))


def test_stale_state_keeps_enable_button_live():
    cfg = SpaceMouseTeleopConfig(deadzone=0.0, input_timeout_s=0.08, require_enable_button=True, enable_button=0)
    state = SimpleNamespace(t=1.0, x=0.0, y=0.0, z=0.0, roll=0.0, pitch=0.0, yaw=0.0, button=[1, 0])

    assert state_to_action(state, cfg, now=1.2)["enabled"] is True
